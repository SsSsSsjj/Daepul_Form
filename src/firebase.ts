...
const formSchema = Schema.object({ properties: {
  ...
  questions: Schema.array({ items: Schema.object({ properties: {
    ...
    }, optionalProperties: ['targetSectionId'] }) }),
...
폼 전체 질문은 6~20개, 한 섹션은 최대 10개로 제한하세요. 하나의 정보를 묻는 질문을 표현만 바꿔 중복 생성하지 마세요.
...
개인정보 질문은 꼭 필요한 최소한만 만드세요. 메모: ${memo || '없음'}`
  ...
  const parsed = JSON.parse(result.response.text()) as Omit<GeneratedForm, 'questions'> & { questions: AiGeneratedQuestion[] }
  const normalized = normalizeGeneratedQuestions(parsed.questions)
  return {
    ...parsed,
    reviewNotes: [...parsed.reviewNotes, ...normalized.reviewNotes],
