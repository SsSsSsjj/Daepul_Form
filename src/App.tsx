...
export default function App() {
  ...
  const startManualForm = () => {
    ...
    setQuestions([{ id: Date.now(), label: '질문을 입력해 주세요', type: 'short_text', required: false }])
    setPage('edit')
  }
  ...
  const duplicateOwnedForm = async (form: OwnedForm) => {
    ...
    try {
      ...
      setQuestions(source.questions.map((question) => ({ ...question, options: question.options ? [...question.options] : undefined })))
...
function QuestionEditor({question,index,count,onChange,onMove,onDelete}:{
  ...
  return <article className="question-editor">
    <div className="question">
      <div className="question-order" aria-label={`${index+1}번 질문 순서 조정`}><span>{index+1}</span><button type="button" aria-label={`${question.label} 위로 이동`} disabled={index===0} onClick={()=>onMove(-1)}><ArrowUp size={15}/></button><button type="button" aria-label={`${question.label} 아래로 이동`} disabled={index===count-1} onClick={()=>onMove(1)}><ArrowDown size={15}/></button></div>
      <input aria-label={`${index+1}번 질문 내용`} value={question.label} onChange={(event)=>onChange({label:event.target.value})}/>
      <select aria-label={`${question.label} 질문 유형`} value={question.type} onChange={(event)=>changeType(event.target.value as FormQuestion['type'])}>{Object.entries(typeLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>
      ...
      <button type="button" aria-label={`${question.label} 삭제`} onClick={onDelete}><Trash2 size={16}/></button>
    </div>
...
function FormBody({program,questions,theme,branding}:{program:ProgramInfo;questions:FormQuestion[];theme:Theme;branding?:FormSettings['branding']}) {
  ...
  return <div className={`form-body theme-${theme}`} style={previewStyle}><FormCover program={program} theme={theme} branding={branding}/>{questions.map((q,i)=><label className="form-question" key={q.id}><span>{i+1}. {q.label} {q.required&&<em>*</em>}</span>{q.type==='long_text'?<textarea disabled/>:q.type==='select'?<select disabled><option>선택해 주세요</option>{(q.options?.length?q.options:['선택지 1','선택지 2']).map(o=><option key={o}>{o}</option>)}</select>:q.type==='rating'?<div className="rating">{[1,2,3,4,5].map(n=><i key={n}>{n}</i>)}</div>:q.type==='checkbox'?<div className="checkbox-options preview-options">{(q.options?.length?q.options:['선택지 1','선택지 2']).map(option=><span className="check-line" key={option}>□ {option}</span>)}</div>:q.type==='consent'?<div className="check-line">□ 동의합니다</div>:q.type==='file'?<div className="file-upload-preview"><Upload/> 파일 선택</div>:<input disabled type={q.inputFormat==='email'?'email':q.inputFormat==='phone'?'tel':q.type==='number'?'number':'text'} placeholder={q.inputFormat==='email'?'name@example.com':q.inputFormat==='phone'?'010-0000-0000':undefined}/>}</label>)}</div>
}
