...
import {
  ...
  manageFormResponses, queryFormResponses, restoreFormRecord, saveAnalysisRecord, saveResponseDraft, setFormCollaborator, signInWithGoogle, summarizeResponses,
...
function QuestionEditor({question,index,count,formId,user,sections,dragging,dropTarget,onDragStart,onDragMove,onDragEnd,onDragCancel,onChange,onMove,onDuplicate,onAdd,onDelete}:{
  ...
  return <article className={`question-editor${dragging?' is-dragging':''}${dropTarget?' is-drop-target':''}`} data-question-id={question.id}>
    <div className="question">
      <div className="question-order"><span aria-hidden="true">{index+1}</span><button type="button" className="question-drag-handle" aria-label={`${questionName} 순서 변경. 현재 ${index+1}번째. 끌어서 이동하거나 키보드 위아래 화살표를 사용하세요.`} onPointerDown={(event)=>{if(event.button!==0)return;event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);onDragStart()}} onPointerMove={(event)=>{if(event.currentTarget.hasPointerCapture(event.pointerId))onDragMove(event.clientX,event.clientY)}} onPointerUp={(event)=>{if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);onDragEnd()}} onPointerCancel={onDragCancel} onKeyDown={(event)=>{if(event.key==='ArrowUp'&&index>0){event.preventDefault();onMove(-1)}else if(event.key==='ArrowDown'&&index<count-1){event.preventDefault();onMove(1)}}}><GripVertical size={20}/></button></div>
    ...
    {question.type==='short_text'&&<div className="response-validation"><label>답변 형식<select value={question.inputFormat??'none'} onChange={(event)=>onChange({inputFormat:event.target.value as FormQuestion['inputFormat']})}><option value="none">제한 없음</option><option value="email">이메일 주소</option><option value="phone">휴대전화 010-0000-0000</option><option value="date">날짜 YYYY-MM-DD</option></select></label><small>{question.inputFormat==='email'?'올바른 이메일 주소가 아니면 제출할 수 없습니다.':question.inputFormat==='phone'?'하이픈(-)을 포함한 010-0000-0000 형식만 허용합니다.':question.inputFormat==='date'?'YYYY-MM-DD 형식의 실제 날짜만 허용합니다.':'필요하면 이메일, 휴대전화 또는 날짜 형식을 지정하세요.'}</small></div>}
  ...
  return <div className={`form-body theme-${theme}`} style={previewStyle}>
    ...
    {visibleQuestions.map((q)=>{
      ...
      return <div className="form-question" key={q.id}><span id={`${inputId}-label`}>{questionIndex+1}. {q.label} {q.required&&<em>*</em>}</span>{q.description&&<small>{q.description}</small>}{q.imageUrl&&<img className="question-response-image" src={q.imageUrl} alt={`${q.label} 질문 이미지`}/>} {q.type==='select'&&q.optionImageUrls?.some(Boolean)&&<div className="option-image-gallery">{options.map((option,index)=>q.optionImageUrls?.[index]?<span key={`${option}-${index}`}><img src={q.optionImageUrls[index]} alt=""/><small>{option}</small></span>:null)}</div>}
        ...
        :<input id={inputId} aria-labelledby={`${inputId}-label`} type={q.inputFormat==='email'?'email':q.inputFormat==='phone'?'tel':q.inputFormat==='date'?'date':q.type==='number'?'number':'text'} placeholder={q.inputFormat==='email'?'name@example.com':q.inputFormat==='phone'?'010-0000-0000':q.inputFormat==='date'?'YYYY-MM-DD':undefined} min={q.min} max={q.max} value={String(answers[q.id]??'')} onChange={event=>updatePreviewAnswer(q,q.type==='number'&&event.target.value!==''?Number(event.target.value):event.target.value)}/>}
      </div>
    })}
...
function PublicForm({user,formId,program,questions,theme,endDate,settings,preview,publicResults,onLogout}:{
  ...
  if(!preview&&loginRequired&&!participantAllowed)return <main className={`public-shell theme-${theme}`}><div className="complete card"><LogIn/><h1>다른 계정으로 로그인이 필요합니다</h1><p>{settings.access.participation==='kangnam'?'인증된 @kangnam.ac.kr 계정으로 다시 로그인해 주세요.':'제작자가 허용한 계정으로 다시 로그인해 주세요.'}</p><div className="complete-actions"><button type="button" className="primary" onClick={onLogout}><LogIn/> 다른 계정으로 로그인</button></div></div></main>
    ...
    <div className="public-form card">
      ...
      {visibleQuestions.map((q)=>{
        ...
        return <div className={`form-question ${fieldError?'invalid':''}`} key={q.id}>
          ...
          :<input id={inputId} aria-labelledby={labelId} aria-invalid={Boolean(fieldError)} aria-describedby={fieldError?`${inputId}-error`:undefined} inputMode={q.inputFormat==='email'?'email':q.inputFormat==='phone'?'tel':q.type==='number'?'numeric':undefined} type={q.inputFormat==='email'?'email':q.inputFormat==='phone'?'tel':q.inputFormat==='date'?'date':q.type==='number'?'number':'text'} placeholder={q.inputFormat==='email'?'name@example.com':q.inputFormat==='phone'?'010-0000-0000':q.inputFormat==='date'?'YYYY-MM-DD':undefined} min={q.min} max={q.max} value={String(answers[q.id]??'')} onChange={e=>updateAnswer(q,q.type==='number'&&e.target.value!==''?Number(e.target.value):e.target.value)}/>}
