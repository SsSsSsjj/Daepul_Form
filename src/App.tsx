...
import { BarChart3, CalendarClock, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, Copy, Download, Eye, FileText, Flower2, GraduationCap, Heart, House, Leaf, LayoutDashboard, LoaderCircle, LogIn, LogOut, Palette, Plus, QrCode, RefreshCcw, Send, Snowflake, Sparkles, Sun, Trash2, Upload, WandSparkles, Waves } from 'lucide-react'
...
import {
  ...
  getDeletedForms, getFormDeliveryStatus, getFormVersions, getOwnResponse, getOwnedForms, getPendingEmailAddress, getPublicResultSummary, getPublishedForm, hasEmailSignInLink,
...
export default function App() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setProgram(generated.program); setQuestions(generated.questions); setFormType(generated.formType); setReviewNotes(generated.reviewNotes); setCreationMode('ai'); setPage('edit')
    } catch (error) {
      console.error(error)
...
function FormBody({program,questions,theme,branding}:{program:ProgramInfo;questions:FormQuestion[];theme:Theme;branding?:FormSettings['branding']}) { return <div className={`form-body theme-${theme}`}><FormCover program={program} theme={theme} branding={branding}/>{questions.map((q,i)=><label className="form-question" key={q.id}><span>{i+1}. {q.label} {q.required&&<em>*</em>}</span>{q.type==='long_text'?<textarea disabled/>:q.type==='select'?<select disabled><option>선택해 주세요</option>{q.options?.map(o=><option key={o}>{o}</option>)}</select>:q.type==='rating'?<div className="rating">{[1,2,3,4,5].map(n=><i key={n}>{n}</i>)}</div>:q.type==='checkbox'||q.type==='consent'?<div className="check-line">□ 동의합니다</div>:q.type==='file'?<div className="file-upload-preview"><Upload/> 파일 선택</div>:<input disabled type={q.type==='number'?'number':'text'}/>}</label>)}</div> }
